...
plot3 <- ggplot(insurance, aes(x=age, y=charges)) +
  ...
  geom_abline(slope = 257.85, intercept = -12102.8 , color = "red", 
              show.legend = TRUE) 
...
print(M2min_max_accuracy) #.457
#-------------------------------------------------
#Currently the information below will not be displayed on dashboard. 
#-------------------------------------------------
...
plot5 <- ggplot(insurance, aes(x=age, y=charges)) +
  geom_point(aes(col=children)) + ggtitle("Charges by Age & Number of Children") + 
  ylab("Charges Applied") + xlab("Age") + theme_bw() + 
  ...
  geom_abline(slope = 254.32, intercept = -373.26, color = "purple", 
              show.legend = TRUE) 
plot5 + theme(plot.title = element_text(hjust = .5))
#---------------------------------------------------
